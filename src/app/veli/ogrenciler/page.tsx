  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-8 rounded-[2.5rem] shadow-sm">
        ...
        <Link href="/veli/ogrenci-ekle">
          <Button size="lg" className="bg-gradient-to-r from-pink-500 to-violet-500 hover:from-pink-600 hover:to-violet-600 text-white font-bold rounded-2xl h-12 px-6 shadow-lg shadow-pink-500/20">
      ...
      {students.length === 0 ? (
        <div className="bg-card rounded-[2.5rem] p-8 border border-border/50 text-center flex flex-col items-center justify-center min-h-[400px]">
           ...
           <Link href="/veli/ogrenci-ekle">
              <Button className="h-12 border-2 border-border font-bold px-8 rounded-xl items-center gap-2 bg-gradient-to-r from-pink-500 to-violet-500 hover:from-pink-600 hover:to-violet-600 text-white">
          ...
          {students.map((student: any) => (
            <div key={student.id} className="bg-card border-2 border-border/50 rounded-3xl p-6 shadow-sm hover:border-violet-500/30 transition-all flex flex-col min-h-[240px]">
               ...
               <div className="mt-auto grid grid-cols-2 gap-3">
                  <Link href={`/ogrenci/sonuclarim?studentId=${student.id}`} className="col-span-1">
                     <Button variant="outline" className="w-full font-bold border-2 hover:bg-muted rounded-xl h-11 text-xs">
                        Raporlar
                     </Button>
                  </Link>
                  <Link href={`/veli/ogrenci-duzenle?id=${student.id}`} className="col-span-1">
                     <Button variant="outline" className="w-full font-bold border-2 hover:bg-muted rounded-xl h-11 text-xs">
                        Düzenle
                     </Button>
                  </Link>
                  <Link href={`/ogrenci?studentId=${student.id}`} className="col-span-2 mt-1">
