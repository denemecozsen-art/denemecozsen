...
export default async function OgrenciDashboardPage(
  ...
  if (userRole === 'parent') {
    ...
    if (!relation) {
      redirect('/veli?error=unauthorized')
    }
  }
  ...
  if (!student) {
    return (
       <div className="flex flex-col items-center justify-center h-[60vh] text-center max-w-lg mx-auto">
          ...
          {userRole === 'parent' && (
             <a href="/veli" className="text-indigo-500 font-bold hover:underline">Veli Paneline Dön</a>
          )}
       </div>
    )
  }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {userRole === 'parent' && (
         <div className="bg-gradient-to-r from-pink-500/10 via-violet-500/10 to-transparent border border-pink-500/30 p-6 rounded-[2rem] flex flex-col md:flex-row items-start md:items-center justify-between gap-5 shadow-sm relative overflow-hidden">
            ...
            <a href="/veli" className="w-full md:w-auto shrink-0 bg-background border-2 border-border hover:border-pink-500/50 text-foreground font-black px-6 py-4 rounded-xl text-sm flex items-center justify-center gap-2 transition-all hover:shadow-lg shadow-sm group">
